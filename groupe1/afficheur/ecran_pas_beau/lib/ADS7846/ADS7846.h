...
typedef struct
{
  uint_least32_t x;
  uint_least32_t y;
} CAL_POINTT; //calibration points for touchpanel
...
typedef struct
{
  ...
  uint_least32_t f;
  uint_least32_t div;
} CAL_MATRIXT; //calibration matrix for touchpanel
...
class ADS7846
{
  public:
    ...
    CAL_MATRIXT tp_matrix;                //calibrate matrix
    ...
    uint_least8_t setCalibration(CAL_POINTT *lcd, CAL_POINTT *tp);
