...
//	explicit XPlaneUdpWorker(const QString& host, int port, QList<DataRef*>& datarefs ) ;
	explicit XPlaneUdpWorker(const QString& host, int recvPort, int sendPort, QList<DataRef*>& datarefs ) ;
  ...
  signals:
	void udpInfo(const QString& ) ;
	void rrefReceived(int id, float value ) ;

  private:
	void setXPlaneTrafic(bool start = true ) ;	
